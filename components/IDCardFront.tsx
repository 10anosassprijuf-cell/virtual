...
const IDCardFront: React.FC<IDCardFrontProps> = ({ data }) => {
  const themes = {
    clean: { bg: '#E9EAEC', gradient: 'linear-gradient(135deg, #E9EAEC 0%, #D1D3D6 100%)', header: '#A3A5A0', footer: '#A4A7A1', textMain: 'text-slate-800', textName: 'text-slate-900', nameBg: 'bg-[#f0f1f3]', photoBorder: 'border-white' },
    black: { bg: '#0a0a0a', gradient: 'linear-gradient(135deg, #1a1a1a 0%, #000000 100%)', header: '#333333', footer: '#000000', textMain: 'text-gray-200', textName: 'text-white', nameBg: 'bg-[#1a1a1a]', photoBorder: 'border-zinc-800' },
    metal: { bg: '#52525b', gradient: 'linear-gradient(135deg, #71717a 0%, #3f3f46 100%)', header: '#27272a', footer: '#18181b', textMain: 'text-zinc-100', textName: 'text-white', nameBg: 'bg-[#27272a]', photoBorder: 'border-zinc-400' },
    rubro: { bg: '#450a0a', gradient: 'linear-gradient(135deg, #7f1d1d 0%, #450a0a 100%)', header: '#991b1b', footer: '#1a0000', textMain: 'text-red-50', textName: 'text-white', nameBg: 'bg-[#310505]', photoBorder: 'border-red-900' }
  };
  ...
  return (
    <div className="relative flex flex-col items-center overflow-hidden m-0 p-0 shadow-2xl" style={{ width: '1080px', height: '1528px', backgroundColor: style.bg, backgroundImage: style.gradient }}>
      ...
      <div className="flex flex-col items-center w-full px-20 pt-10 flex-1 relative">
        {/* Espaço para Brasão Personalizado */}
        <div className="w-[320px] h-[320px] mb-6 relative flex items-center justify-center">
          {data.brandLogoUrl ? (
            ...
            <div className="w-full h-full border-4 border-dashed border-black/10 rounded-full flex items-center justify-center">
               <span className="text-[20px] font-black text-black/10 uppercase tracking-widest text-center px-8">Anexe seu Brasão</span>
            </div>
          )}
        </div>

        {/* Título Polícia Penal */}
        ...
        <div className="text-center w-full px-2">
          <p className={`text-[35px] font-bold uppercase tracking-[0.5em] mb-4 opacity-90 ${style.textMain}`}>{data.category}</p>
          <div className={`rounded-[40px] min-h-[160px] w-full flex items-center justify-center py-8 px-12 shadow-2xl ${style.nameBg}`}>
            <p className={`${getNameFontSize(data.fullName)} font-black uppercase leading-[1.0] tracking-tighter text-center ${style.textName}`}>
              {data.fullName || "NOME DO TITULAR"}
            </p>
          </div>
        </div>
      </div>
